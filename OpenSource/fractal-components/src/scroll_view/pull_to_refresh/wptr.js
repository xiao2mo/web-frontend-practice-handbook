...
export default function WebPullToRefresh() {
  ...
  var _panStart = function (e) {
    pan.startingPositionY = !!options.bodyEl.getAttribute("inner-scroll-top") ? options.bodyEl.getAttribute("inner-scroll-top") : options.bodyEl.scrollTop;

    // 判断是否允许下拉刷新
    if (pan.startingPositionY < 10) {
      pan.enabled = true;
    } else {
      pan.enabled = false;
    }
