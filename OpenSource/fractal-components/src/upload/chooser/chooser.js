...
export default class UploadChooser extends Component {
  ...
  _onChange = () => {
    ...
    for (let index = 0; index < files.length; index++) {
      if (existedFiles.indexOf(files[index].name) == -1) { // 如果未发现重名
  ...
  render() {
    ...
    if (!this.props.children) {
      children = <BlueWhiteRaisedButton className="upload_chooser__button" label={label} reverse={false}/>
    }

    return <div className="upload_chooser__container">
      {children}
      {/*隐藏起来的文件上传表单*/}
      <form ref="form">
        {
          multiple ?
            <input className="upload_chooser__input"
                   accept={accept}
